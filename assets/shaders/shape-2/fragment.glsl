...
float function(vec3 position) {
  ...
  return opU(sphere(pos, 0.58) - 0.38 * snoise(vec3(pos * 4.5 + time * 1.5)), udRoundBox(position - vec3(0.0, 0.0, 4.0), vec3(8.0, 4.0, 1.0), 0.2));
}
...
vec3 gradient(vec3 position) {
  ...
  return vec3(function(position + vec3(DELTA, 0.0, 0.0)) - function(position - vec3(DELTA, 0.0, 0.0)), function(position + vec3(0.0, DELTA, 0.0)) - function(position - vec3(0.0, DELTA, 0.0)), function(position + vec3(0.0, 0.0, DELTA)) - function(position - vec3(0.0, 0.0, DELTA)));

}

void mainImage(out vec4 fragColor, in vec2 fragCoord) {

  vec3 nearPlanePosition = vec3((fragCoord.x - 0.5 * width) / width * 2.0 * aspect, (fragCoord.y - 0.5 * height) / height * 2.0, 0.0);
